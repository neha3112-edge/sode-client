...
export async function generateMetadata({ params }) {
  ...
  if (slugArray.length === 1) {
    ...
    try {
      const data = await getUniversityData(uniSlug);
      if (!data) {
        return {
      ...
      const canonical = `https://mysode.com/universities/${data.slug || uniSlug}`;
  ...
  try {
    const course = await getCourseData(slugArray);
    ...
    const canonical = `https://mysode.com/university/${course?.slug || slugStr}`;
