...
function processArticleContent(rawHtml, bannerUrl, relatedEntities = []) {
  ...
  // 🖼️ Proxy MinIO image URLs to Next.js middleware proxy route (/media/...)
  clean = clean
...
export default function BlogDetailPage() {
  ...
  // 1. Fetch Blog Data Strictly from CRM Backend API
  useEffect(() => {
    let isMounted = true;
    if (slug) {
      setLoading(true);
      ...
      request
        .dynamicRead({ entity: "blogs", endPoint: "v1/list", id: slug })
        .then((res) => {
          if (!isMounted) return;
        ...
        .catch((err) => {
          console.error("Error loading blog details from CRM:", err);
        })
        .finally(() => {
          if (isMounted) setLoading(false);
        });

      // Fetch Most Popular Blogs from CRM Backend API
      request
        .dynamicList({ entity: "blogs", endPoint: "v1/list", options: { items: 6 } })
        .then((res) => {
          if (!isMounted) return;
          const list = res?.result || res?.blogs || (Array.isArray(res) ? res : []);
          if (Array.isArray(list)) {
            setPopularBlogs(list.filter((b) => b.slug !== slug));
          }
        })
        .catch(() => {});
    } else {
  ...
  const stats = pageData?.stats || blog.stats || [];
  ...
  // 🧭 Dynamically extract Table of Contents (TOC) from <h2> tags
  ...
  // Inject IDs into <h2> tags for smooth TOC scrolling
  ...
  return (
    <WebsiteLayout py="py-4 sm:py-6" bg="#f8fafc">
      <div className="max-w-7xl mx-auto space-y-5 px-3 sm:px-4 md:px-6">
        ...
        {loading ? (
          <div className="bg-white p-6 md:p-10 rounded-2xl border border-slate-200 space-y-4">
          ...
          /* 📰 2-Column Responsive Layout (Main 8 Cols | Sidebar 4 Cols) */
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-7 items-start">
            ...
            <article className="lg:col-span-8 bg-white rounded-2xl border border-slate-200/90 shadow-xs p-5 sm:p-7 md:p-9 space-y-6">
              ...
              {/* Meta Row: Author, Date, Read Time, Social Sharing */}
              ...
              {/* 📑 Dynamic Table of Contents (TOC) Box */}
              {tocItems.length > 0 && (
              ...
              {/* 📖 Dynamic Article Content rendered via SafeHtmlRenderer with Next.js Proxy & Image Optimization */}
              {processedContent ? (
                <div
              ...
              {/* 📊 Structured Salary Progression Tables (if present) */}
              ...
              {/* 🎯 Structured Specializations Factors (if present) */}
            ...
            {/* 🧭 RIGHT SIDEBAR (Sticky FormWrapper & Popular Blogs) */}
            <aside className="lg:col-span-4 space-y-6 lg:sticky lg:top-5">
              {/* 1. 📝 Shared FormWrapper Lead Form Component */}
              <div className="bg-white rounded-2xl border border-slate-200 p-2 sm:p-3 shadow-sm">
                <FormWrapper
                  ...
                  defaultCourse="MBA"
                  formNameOverride={`BlogDetail_${slug}`}
                  submitButtonText="Submit"
                />
              </div>

              {/* 2. 📰 "Most Popular Blogs" Widget */}
