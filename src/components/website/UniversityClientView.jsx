    ...
    <div className="min-h-screen bg-gray-100 text-gray-800 antialiased font-sans pb-16">
      ...
      >
        {selectedCourseSpec && (() => {
          ...
          const uniLogoSrc = logoUrl || courseLogoUrl;
          ...
          return (
            <div className="flex flex-col md:flex-row items-stretch gap-4 sm:gap-6 p-0 sm:p-1 pb-1 max-h-[85vh] overflow-y-auto overflow-x-hidden">
              ...
              <div className="relative w-full md:w-80 lg:w-88 shrink-0 h-36 sm:h-44 md:h-auto min-h-36 sm:min-h-44 md:min-h-[350px] md:max-h-[460px] rounded-xl sm:rounded-2xl overflow-hidden bg-slate-900 shadow-xs flex items-stretch">
                ...
                {uniLogoSrc && (
                  <div className="absolute top-2.5 left-2.5 sm:top-3.5 sm:left-3.5 z-10 bg-white rounded-lg sm:rounded-xl shadow-md p-1.5 sm:p-2 flex flex-col items-center justify-center text-center border border-gray-100/90 min-w-[60px] sm:min-w-[74px] max-w-[76px] sm:max-w-[90px]">
                    ...
                    <span className="text-[8px] sm:text-[9.5px] font-bold text-gray-900 leading-tight mt-0.5 sm:mt-1 line-clamp-1 sm:line-clamp-2">
                      {uniName}
                    </span>
                  </div>
                )}
              ...
              <div className="flex-1 flex flex-col justify-between text-left gap-3 md:min-h-[350px]">
                {/* Top Details */}
                <div className="flex flex-col gap-2 sm:gap-2.5">
                  {/* University Name Badge */}
                  {uniName && (
                    <div>
                      <span className="inline-block bg-[#E59819] text-white text-[10px] sm:text-[11px] font-bold px-2.5 py-0.5 rounded-md uppercase tracking-wider">
                        {uniName}
                      </span>
                    </div>
                  )}
                  ...
                  {/* Meta Info Row */}
                  <div className="flex flex-wrap items-center gap-x-3.5 gap-y-1.5 text-xs text-gray-500 font-medium">
                    {cDuration && (
                      <div className="flex items-center gap-1.5">
                        <FaClock className="w-3.5 h-3.5 text-[#072C50] shrink-0" />
                        <span>{cDuration}</span>
                      </div>
                    )}
                    {modalFeeText && (
                      <span className="font-bold text-gray-700">{modalFeeText}</span>
                    )}
                    {itemDeadline && (
                      <div className="flex items-center gap-1.5">
                        <FaCalendar className="w-3.5 h-3.5 text-[#072C50] shrink-0" />
                        <span className="text-red-500">
                          Admission Deadline : <strong className="font-semibold text-gray-600">{itemDeadline}</strong>
                        </span>
                      </div>
                    )}
                  </div>
