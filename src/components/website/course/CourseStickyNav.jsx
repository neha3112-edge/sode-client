...
const cleanLabelFromText = (text = "") => {
  if (!text) return "";
  // Strip common trailing years (e.g. "2026", "2025") or repetitive prefixes if needed
  return text.trim();
};
...
export default function CourseStickyNav({
  containerId = "course-content-sections",
  sections = null,
  courseName = "",
}) {
  ...
  // Initial sections from props (for SSR without static candidate fallbacks)
  ...
  const scanDomSections = useCallback(() => {
    ...
    const targetContainer =
      ...
      document.getElementById("course-content-sections") ||
      document.querySelector("main");

    if (!targetContainer) return;

    // Find all direct child divs/sections or elements marked with an ID
    ...
    candidateNodes.forEach((node) => {
      ...
      // Ignore non-section elements like scripts, modals, or zero-height hidden items
      if (node.tagName === "SCRIPT" || node.tagName === "TEMPLATE") return;

      // Check for explicit nav label attribute first
      let label =
        node.getAttribute("data-nav-label") ||
        node.getAttribute("data-title") ||
        node.getAttribute("data-label");

      // Fallback: extract from inner heading (h2, h3, h1)
      if (!label) {
      ...
      // Fallback: humanize the id attribute
      if (!label) {
  ...
  // Scan immediately on mount and observe DOM changes (e.g. backend dynamic sections)
  useEffect(() => {
    scanDomSections();
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // If reached the bottom of page, highlight the last section
      if (
        ...
        <div className="grow overflow-x-auto min-w-0">
          ...
          >
            <Tabs
              ...
              className="course-sticky-nav-tabs [&_.ant-tabs-nav]:mb-0 [&_.ant-tabs-tab]:py-3 [&_.ant-tabs-tab-btn]:font-medium [&_.ant-tabs-tab-active_.ant-tabs-tab-btn]:font-bold [&_.ant-tabs-tab-active_.ant-tabs-tab-btn]:text-[#0C2B4E] [&_.ant-tabs-ink-bar]:bg-[#0C2B4E]"
            />
          </ConfigProvider>
        </div>
      </div>
