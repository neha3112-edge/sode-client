      ...
      <div className="hidden sm:flex items-center justify-between gap-3 w-full">
            ...
            return (
              ...
              >
                {logo ? (
                  ...
                  <div className="w-4 h-4 rounded-full bg-blue-100 text-blue-800 text-[9px] font-bold flex items-center justify-center">
                    {displayName.charAt(0)}
                  </div>
                )}
                <span className="text-[11px] font-bold text-slate-700 truncate max-w-36">
                  {displayName}
                </span>
                <button
                  type="button"
                  onClick={() => removeFromCompare(itemKey)}
                  className="text-slate-400 hover:text-red-600 cursor-pointer ml-0.5 p-0.5 rounded-full transition-colors border-none bg-transparent"
                  title="Remove item"
                >
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <button
            ...
            className="text-xs font-bold text-slate-400 hover:text-red-600 cursor-pointer px-2 transition-colors flex items-center gap-1 border-none bg-transparent"
          >
            Clear
          </button>
      ...
      <div className="flex sm:hidden flex-col gap-2.5 w-full">
        ...
        <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2">
                ...
                return (
                  ...
                  >
                    {logo ? (
                      ...
                      <div className="w-full h-full text-[9px] font-bold text-blue-700 flex items-center justify-center">
                        {displayName.charAt(0)}
                      </div>
                    )}
                  </div>
          ...
          <button
            type="button"
            onClick={clearCompare}
            className="text-[11px] font-bold text-slate-400 hover:text-red-600 transition-colors border-none bg-transparent"
          >
            Reset
          </button>
        </div>
        ...
        <Link href="/compare" className="w-full">
          ...
          >
            Compare Selected ({compareList.length}) <RightOutlined className="text-[10px]" />
          </Button>
        </Link>
      </div>
